...
class TestHardwareViewEditPageHelper:
    def login_to_app(self, driver, username, password, return_object, url=ReadConfig.get_application_url()):
        ...
        self.hardwarevieweditpage = self.loginPage.do_login(username, password, return_object)

    def open_hardware_view_page(self, path, environment):
        url = self.url_builder(path, environment)
        self.hardwarevieweditpage.navigate_to_hardware_view_page(url)
    ...
    def check_if_click_to_view_password_link_present(self):
        return self.hardwarevieweditpage.check_for_mask_password_link()
